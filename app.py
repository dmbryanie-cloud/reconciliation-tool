...
def qbo_token():
    ...
    for rt in candidates:
        try:
            ...
            _store_refresh(
                result.get("refresh_token", rt)
            )  # persist whatever QBO returns
...
def ingest_csv(text, account_name):
    ...
    if not (dk and ak):
        raise ValueError(
            f"CSV needs Date and Amount columns. Found: {reader.fieldnames}"
        )
    rows = []
    for r in reader:
...
def run_matcher(statement_id):
    ...
    def wm(lids, tids, mt, conf, delta):
        cur.execute(
            "INSERT INTO match (org_id, statement_id, status, match_type, confidence, amount_delta) VALUES (%s,%s,'confirmed',%s,%s,%s) RETURNING match_id;",
    ...
    for l_id, ld, la, lw in lines:
        ...
        if found:
            ...
            wm([l_id], tids, "many_to_one", 0.8, 0)
            matched_lines.add(l_id)
            used.update(tids)
    conn.commit()
    cur.close()
